    ...
    if (
      ...
      if (emailvalidator.validate(req.body.email)) {
        ...
        if (passwordCheck.length >= 8) {
          let mobNumbCheck = req.body.phoneNumber
            .replace(/[- )(]/g, "")
            .split("");
          ...
          if (mobNumbCheck.length === 10) {
            profileService.createProfile(
              ...
              async function (err, result) {
                if (err) {
                  fs.unlinkSync(
                    "./tmp/" + req.file.filename.replace(/\s/g, "")
                  );
                  console.log("File is deleted.");
                  return res
                    .status(400)
                    .json({ status: "error", data: { error: err } });
                }
          ...
          res
            .status(400)
            .send("Password length should be minimum 8 characters");
        }
      } else {
        fs.unlinkSync("./tmp/" + req.file.filename.replace(/\s/g, ""));
        console.log("File is deleted.");
